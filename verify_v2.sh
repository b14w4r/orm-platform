...
# verify.sh v2
# Usage:
#   ./verify.sh tests   -> runs integration tests (Testcontainers)
#   ./verify.sh smoke   -> starts postgres (docker compose) + app + runs 3 scenario curl checks

MODE="${1:-tests}"  # tests | smoke
LOG_FILE="${LOG_FILE:-./target/app.log}"
...
if [[ "$MODE" == "tests" ]]; then
  echo "==> Running mvn test (uses Testcontainers; Docker must be available)"
  mvn -q test
  echo "OK"
  exit 0
fi
...
# Prepare log file (fixes 'app.log: Permission denied' when old file is root-owned)
mkdir -p "$(dirname "$LOG_FILE")"
: > "$LOG_FILE"

# Check Docker access early
if ! docker info >/dev/null 2>&1; then
  echo "Docker is not доступен текущему пользователю. Проверь группу docker/права на /var/run/docker.sock" >&2
  exit 1
fi
...
echo "==> Waiting for app on http://localhost:8080 (OpenAPI endpoint) ..."
for i in {1..90}; do
  ...
  sleep 1
  if [[ $i -eq 90 ]]; then
    echo "App did not become ready. Check logs: $LOG_FILE" >&2
    exit 1
  fi
done
